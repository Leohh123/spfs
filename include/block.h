#ifndef __SPFS_BLOCK_H__
#define __SPFS_BLOCK_H__
...
static void read_complete(void *arg, const struct spdk_nvme_cpl *completion) {
    struct spfs_sequence *sequence = arg;
...
static int spfs_write_block(void *input, uint64_t blkno) {
    // printf("[call] spfs_write_block: blkno = %d\n", blkno);
    struct ns_entry *ns_entry;
    struct spfs_sequence sequence;
    int rc;
    size_t sz;
    ...
    TAILQ_FOREACH(ns_entry, &g_namespaces, link) {
        ...
        if (sequence.buf == NULL || sz < SPFS_BLKSZ) {
            sequence.using_cmb_io = 0;
            sequence.buf = spdk_zmalloc(SPFS_BLKSZ, SPFS_BLKSZ, NULL, SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_DMA);
        }
        ...
        // if (sequence.using_cmb_io) {
        //     printf("INFO: using controller memory buffer for write\n");
        // } else {
        //     printf("INFO: using host memory buffer for write\n");
        // }
        sequence.is_completed = 0;
        sequence.ns_entry = ns_entry;

        // Write to buffer
        // snprintf(sequence.buf, SPFS_BLKSZ, "%s", (char *)input);
        memcpy(sequence.buf, input, SPFS_BLKSZ);
        ...
        // Wait until the write I/O completes
...
static int spfs_read_block(void **output, uint64_t blkno) {
    struct ns_entry *ns_entry;
    struct spfs_sequence sequence;
    int rc;
    ...
    TAILQ_FOREACH(ns_entry, &g_namespaces, link) {
        ...
        // printf("INFO: using host memory buffer for read\n");
        sequence.is_completed = 0;
        sequence.ns_entry = ns_entry;
        ...
        // Wait until the read I/O completes
        while (!sequence.is_completed) {
            spdk_nvme_qpair_process_completions(ns_entry->qpair, 0);
        }
...
static void spfs_finish(void) {
    cleanup();
    spdk_env_fini();
}

static int spfs_init(int argc, char **argv) {
    int rc;
